...
var HRRN = (function () {
  function responseRatio(waitTime, bt) {
    return (waitTime + bt) / bt;
  }
  ...
  function run(processes, config) {
    ...
    while (done.length < procs.length && time < maxTime) {
      ...
      // Sort by response ratio (descending)
      readyQueue.sort(function (a, b) {
        var waitA = time - a.at - (a.bt - a.remaining);
        var waitB = time - b.at - (b.bt - b.remaining);
        return responseRatio(waitB, b.bt) - responseRatio(waitA, a.bt);
      });
